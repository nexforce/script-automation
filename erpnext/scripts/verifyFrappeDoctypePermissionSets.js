  ...
  for (const batch of segmentArray(doctypes, 25))
    await Promise.all(
      batch.map(async (doctype) => {
        ...
        if (!docs.length) {
          console.log(getBody);
          console.error(`Not found: ${doctype}`);
          return;
        }

        const [{ permissions }] = docs;

        if (
          permissions.length === 2 &&
          permissions.some((d) => d.role === "Administrator") &&
          permissions.some((d) => d.role === "System Manager")
        )
          return;

        console.log(getBody);
        console.log(`Did not match: ${doctype}`);
      })
    );
